...
class RoundService
{
    ...
    public function calculateRound(): array
    {
        ...
        /**
         * if there is no contest id,
         * we dont have a contest and contest_rounds to calculate and update
         */
        if (!$contestId) {
            return [];
        }
        ...
        foreach ($contestantsData as $contestantData) {
             ...
             */
            $genreScore = $genreService->genreCalculation($contestantData, (int)$roundGenreId);
            $contestantData->contestantScore = $genreScore;
            ...
            if($contestantIsSick){
                $genreScore = round(($genreScore / 2), 1);
            }

            /**
             * get contest judges and calculate judges score
             */
            $judgeScores = $judgeService->judgeCalculation($genreScore, $contestantIsSick, (int)$roundGenreId);
     ...
     * @throws \Exception
     */
    public function createRoundLog(array $roundData): void
    {
        $roundLog = new RoundLog();
        $roundDataColumns = array_merge(
            ['contest_id'   => $roundData['contestId']],
            ['round_id'     => $roundData['roundId']],
            ['round'        => $roundData['round']],
            ['json_data'    => json_encode($roundData)]
        );

        $roundLog->add($roundDataColumns);
    }

    /**
     * prepare round log
     */
    public function roundLog(int $contestId): void
    {
        $roundDataGoingOn = $this->getRoundData($contestId);

        if ($roundDataGoingOn) {
            $this->createRoundLog($roundDataGoingOn);
        }
    }
}
